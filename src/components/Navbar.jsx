...
import { Truck, Bell, Moon, Sun, Menu } from 'lucide-react';
import JerehLogo from '../assets/jereh.png';

export default function Navbar({ companyName, currentPage, setCurrentPage, theme, setTheme, sidebarOpen, setSidebarOpen }) {
  ...
  return (
    <header className="bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-700 h-16 min-h-16 flex items-center justify-between px-6 z-30 select-none">
      ...
      <div className="flex items-center space-x-3">
        <button
          type="button"
          className="p-2 rounded-xl text-slate-500 hover:bg-slate-100 dark:hover:bg-slate-800 dark:text-slate-300 dark:hover:text-white transition-all duration-300 lg:hidden"
          onClick={() => setSidebarOpen(!sidebarOpen)}
          aria-label="Toggle navigation menu"
        >
          <Menu className="h-5 w-5" />
        </button>
        <div className="flex items-center space-x-4 cursor-pointer" onClick={() => setCurrentPage('Dashboard')}>
          {companyName === 'Jereh' ? (
            <img src={JerehLogo} alt="Jereh Logo" className="h-30 w-auto object-contain" />
          ) : (
