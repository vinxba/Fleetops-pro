...
export default function FleetPage({ vehiclesData, initialVehicleId, onClearSelection }) {
  ...
  const getVehicleImage = (vehicleType) => {
    return typeImageMap[vehicleType] || MitsubishiL200;
  };

  const mapVehicleResponse = (vehicle) => {
    const fuel = Number(vehicle.fuel_level) || 0;
    const oil = Number(vehicle.oil_level) || 0;
    const battery = Number(vehicle.battery_health) || 0;
    const monthlyMileage = Number(vehicle.monthly_mileage) || (Number(vehicle.monthly_end_mileage) - Number(vehicle.monthly_start_mileage)) || 0;
    ...
    return {
      ...
      image: imageUrl,
      metrics: [
        { label: 'CURRENT MILEAGE', value: `${Number(vehicle.monthly_end_mileage).toLocaleString()} KM` },
        { label: 'MONTHLY MILEAGE', value: `${monthlyMileage.toLocaleString()} KM` },
        { label: 'FUEL LEVEL', value: `${fuel.toFixed(0)}%`, progress: Math.min(100, Math.max(0, fuel)), progressColor: 'bg-emerald-600' },
        { label: 'OIL LEVEL', value: `${oil.toFixed(0)}%`, progress: Math.min(100, Math.max(0, oil)), progressColor: 'bg-slate-600' },
      ],
      repairHistory: [],
      partsRemoved: [],
      technicalSpecs: [
        { label: 'Type', value: vehicleType },
        { label: 'Battery Health', value: `${battery.toFixed(0)}%` },
        { label: 'Fuel Level', value: `${fuel.toFixed(0)}%` },
        { label: 'Oil Level', value: `${oil.toFixed(0)}%` },
        { label: 'Driver', value: vehicle.assigned_driver || 'Unassigned' },
      ],
      footerLeft: { label: 'NEXT SERVICE', value: vehicle.next_service_date || 'Not scheduled' },
